...
// ============================================
// STEP 1: CORE SETUP AND HELPER FUNCTIONS
// ============================================
// ✓
// generate_pdf = async (req, res) => {
//   try {
...
//       if (!currentPage || y - requiredSpace < MIN_BOTTOM_MARGIN) {
//         addNewPage();
//       }
//     };

//     const moveDown = (space) => {
//       y -= space;
//       checkSpace(100);
//     };
...
//     function drawTableWithBlueHeader(page, x, yPos, width, rows, font, fontBold, blueColor) {
//       const rowHeight = 30;
//       let currentY = yPos;
...
//     function drawSimpleTable(page, x, yPos, width, rows, font, fontBold) {
//       const rowHeight = 25;
//       const colSplit = width * 0.35;
//       let currentY = yPos;
...
//     function drawEmissionsTable(page, x, yPos, width, emissions, font, fontBold, blueColor) {
//       const headers = ["EMISSIONS", "TOTAL\n(mt CO2e)", "CO2\n(mt)", "CH4\n(mt)", "N2O\n(mt)", "HFCs\n(mt)", "PFCs\n(mt)", "SF6\n(mt)"];
//       const colWidth = width / 8;
//       const rowHeight = 30;
//       let currentY = yPos;
...
//       const rows = [
//         ["Scope 1", emissions.scope1Total, emissions.scope1CO2, emissions.scope1CH4, emissions.scope1N2O, emissions.scope1HFCs, emissions.scope1PFCs, emissions.scope1SF6],
//         ["Scope 2", emissions.scope2Total, emissions.scope2CO2, emissions.scope2CH4, emissions.scope2N2O, emissions.scope2HFCs, emissions.scope2PFCs, emissions.scope2SF6],
//         ["Scope 3\n(OPTIONAL)", emissions.scope3Total, emissions.scope3CO2, emissions.scope3CH4, emissions.scope3N2O, emissions.scope3HFCs, emissions.scope3PFCs, emissions.scope3SF6]
//       ];
...
//     function drawSingleRowTable(page, x, yPos, width, headerText, value, font, fontBold, blueColor) {
//       let currentY = yPos;
//       const maxHeaderWidth = width - 10;
//       const words = headerText.split(' ');
//       let lines = [];
...
//     function drawOrganizationalTable(page, x, yPos, width, boundaries, font, fontBold, blueColor) {
//       const colWidths = [width * 0.4, width * 0.2, width * 0.2, width * 0.2];
//       const headerHeight = 50;
//       const dataRowHeight = 30;
//       let currentY = yPos;
...
//           page.drawLine({
//             start: { x: currentX + colWidths[idx], y: currentY },
//             end: { x: currentX + colWidths[idx], y: currentY - headerHeight - (dataRowHeight * boundaries.length) },
//             thickness: 0.5,
//             color: black
//           });
//         }
...
//         });
//         currentX = x;
//         [boundary.entity || "", boundary.equityShare || "", boundary.financialControl || "", boundary.operationalControl || ""].forEach((value, idx) => {
//           page.drawText(String(value), {
//             x: currentX + 3,
//             y: currentY - 18,
//             size: 8,
...
//     function drawDetailedEmissionsTable(page, x, yPos, width, rows, font, fontBold, blueColor) {
//       const rowHeight = 20;
//       const colSplit = width * 0.75;
//       let currentY = yPos;
...
//     function drawFacilityTableWithHeader(page, x, yPos, width, headerText, facilities, font, fontBold, blueColor) {
//       const rowHeight = 22;
//       let currentY = yPos;
//       const maxHeaderWidth = width - 10;
//       const words = headerText.split(' ');
...
//       page.drawText("Facility", { x: x + 5, y: currentY - 15, size: 9, font: fontBold });
//       page.drawText("Scope 1 emissions", { x: x + colWidths[0] + 5, y: currentY - 15, size: 9, font: fontBold });
...
//           borderColor: black
//         });
//         page.drawText(String(f.facility || ""), { x: x + 5, y: currentY - 14, size: 8, font });
//         page.drawText(String(f.scope1Emissions || ""), { x: x + colWidths[0] + 5, y: currentY - 14, size: 8, font });
//         currentY -= rowHeight;
//       });
//     }

//     function drawCountryTableWithHeader(page, x, yPos, width, headerText, countries, font, fontBold, blueColor) {
//       const rowHeight = 22;
//       let currentY = yPos;
//       const maxHeaderWidth = width - 10;
//       const words = headerText.split(' ');
...
//       page.drawText("Country", { x: x + 5, y: currentY - 15, size: 9, font: fontBold });
//       page.drawText("Emissions (specify Scopes included)", { x: x + colWidths[0] + 5, y: currentY - 15, size: 8, font: fontBold });
...
//           borderColor: black
//         });
//         page.drawText(String(c.country || ""), { x: x + 5, y: currentY - 14, size: 8, font });
//         page.drawText(String(c.emissions || ""), { x: x + colWidths[0] + 5, y: currentY - 14, size: 8, font });
//         currentY -= rowHeight;
//       });
//     }

//     function drawOffsetsTableWithHeader(page, x, yPos, width, headerText, rows, font, fontBold, blueColor) {
//       const rowHeight = 22;
//       let currentY = yPos;
//       const maxHeaderWidth = width - 10;
//       const words = headerText.split(' ');
...
//           page.drawLine({
//             start: { x: currentX + colWidths[idx], y: currentY },
//             end: { x: currentX + colWidths[idx], y: currentY - columnHeaderHeight - rowHeight * rows.length },
//             thickness: 0.5,
//             color: black
//           });
//         }
...
//     // ============================================
//     // STEP 2: MAIN CONTENT GENERATION
//     // ============================================
...
//     if (data.inventoryYear) {
//       const yearWidth = fontBold.widthOfTextAtSize(data.inventoryYear, 14);
//       currentPage.drawText(data.inventoryYear, {
...
//           const scaledWidth = imgDims.width * scale;
//           const scaledHeight = imgDims.height * scale;
//           const imgX = MARGIN + (boxWidth - scaledWidth) / 2;
//           const imgY = y - 80 + (boxHeight - scaledHeight) / 2;
...
//       currentPage.drawText(verHeader, {
//         x: MARGIN + (CONTENT_WIDTH - hdrWidth) / 2,
//         y: y - 22,
//         size: 10,
//         font: fontBold
//       });
...
//       currentPage.drawText(data.verified ? "[ ] No" : "[✓] No", { 
//         x: MARGIN + 20, 
//         y: y - 17, 
//         size: 10, 
//         font 
...
//         data.verified ? "[✓] Yes (if yes, fill verification information below)" : "[ ] Yes (if yes, fill verification information below)",
//         { x: MARGIN + 150, y: y - 17, size: 10, font }
//       );
//       moveDown(optionHeight + 15);
...
//         if (verificationRows.length > 0) {
//           checkSpace(verificationRows.length * 25 + 30);
//           drawSimpleTable(currentPage, MARGIN, y, CONTENT_WIDTH, verificationRows, font, fontBold);
//           moveDown(verificationRows.length * 25 + 20);
//         }
//       }
//     }
...
//       checkSpace(100);
//       moveDown(20);
//       drawSingleRowTable(currentPage, MARGIN, y, CONTENT_WIDTH,
//         "Have any facilities, operations and/or emissions sources been excluded from this inventory? If yes, please specify.",
//         data.exclusions, font, fontBold, lightBlue);
//       moveDown(80);
//     }
...
//       moveDown(20);
//       const periodText = `From ${data.periodFrom || "MM/DD/YYYY"} to ${data.periodTo || "MM/DD/YYYY"}`;
//       drawTableWithBlueHeader(currentPage, MARGIN, y, CONTENT_WIDTH,
//         [
//           { label: "Reporting period covered by this inventory", value: "", isHeader: true },
//           { label: periodText, value: "", isData: true }
//         ],
//         font, fontBold, lightBlue);
//       moveDown(80);
//     }
...
//       currentPage.drawText("ORGANIZATIONAL BOUNDARIES", {
//         x: MARGIN,
//         y,
//         size: 14,
//         font: fontBold
...
//       currentPage.drawText(option1, { x: MARGIN + 20, y: y - 20, size: 10, font });
//       currentPage.drawText(option2, { x: MARGIN + 170, y: y - 20, size: 10, font });
//       currentPage.drawText(option3, { x: MARGIN + 340, y: y - 20, size: 10, font });
//       moveDown(checkboxRowHeight + 20);
//     }
...
//     currentPage.drawText("OPERATIONAL BOUNDARIES", {
//       x: MARGIN,
//       y,
//       size: 14,
//       font: fontBold
...
//       drawTableWithBlueHeader(currentPage, MARGIN, y, CONTENT_WIDTH,
//         [
//           { label: "Are Scope 3 emissions included in this inventory?", value: "", isHeader: true },
//           { label: data.scope3Included ? "[✓] yes  [ ] no" : "[ ] yes  [✓] no", value: "", isData: true }
//         ],
//         font, fontBold, lightBlue);
//       moveDown(80);
//     }
...
//       currentPage.drawText("If yes, which types of activities are included in Scope 3 emissions?",
//         { x: MARGIN + 5, y: y - 20, size: 10, font: fontBold, maxWidth: CONTENT_WIDTH - 10 });
//       moveDown(scope3HeaderHeight);
...
//       currentPage.drawText(String(data.scope3Activities), {
//         x: MARGIN + 5,
//         y: y - 15,
//         size: 9,
//         font,
...
//       currentPage.drawText("INFORMATION ON EMISSIONS", {
//         x: MARGIN,
//         y,
//         size: 14,
//         font: fontBold
...
//       currentPage.drawText("The table below refers to emissions independent of any GHG trades such as",
//         { x: MARGIN, y, size: 9, font });
//       moveDown(12);
//       currentPage.drawText("sales, purchases, transfers, or banking of allowances",
//         { x: MARGIN, y, size: 9, font });
//       moveDown(25);

//       drawEmissionsTable(currentPage, MARGIN, y, CONTENT_WIDTH, data.emissions || {}, font, fontBold, lightBlue);
//       moveDown(115);
//     }
...
//       checkSpace(60);
//       drawTableWithBlueHeader(currentPage, MARGIN, y, CONTENT_WIDTH,
//         [{ label: "Direct CO2 emissions from Biogenic combustion (mtCO2)", value: data.biogenicEmissions, isHeader: true }],
//         font, fontBold, lightBlue);
//       moveDown(60);
//     }
...
//       currentPage.drawText("BASE YEAR", {
//         x: MARGIN,
//         y,
//         size: 14,
//         font: fontBold
...
//       if (data.baseYear) {
//         checkSpace(80);
//         drawSingleRowTable(currentPage, MARGIN, y, CONTENT_WIDTH, "Year chosen as base year", data.baseYear, font, fontBold, lightBlue);
//         moveDown(80);
//       }
...
//         drawSingleRowTable(currentPage, MARGIN, y, CONTENT_WIDTH,
//           "Clarification of company-determined policy for making base year emissions recalculations",
//           data.baseYearPolicy, font, fontBold, lightBlue);
//         moveDown(80);
//       }
...
//         drawSingleRowTable(currentPage, MARGIN, y, CONTENT_WIDTH,
//           "Context for any significant emissions changes that trigger base year emissions recalculations",
//           data.baseYearContext, font, fontBold, lightBlue);
//         moveDown(80);
//       }
...
//         checkSpace(180);
//         drawTableWithBlueHeader(currentPage, MARGIN, y, CONTENT_WIDTH,
//           [{ label: "Base year emissions", value: "", isHeader: true }],
//           font, fontBold, lightBlue);
//         moveDown(40);
//         drawEmissionsTable(currentPage, MARGIN, y, CONTENT_WIDTH, data.baseYearEmissions, font, fontBold, lightBlue);
//         moveDown(140);
//       }
//     }
...
//       currentPage.drawText("METHODOLOGIES AND EMISSION FACTORS", {
//         x: MARGIN,
//         y,
//         size: 14,
//         font: fontBold
...
//       drawSingleRowTable(currentPage, MARGIN, y, CONTENT_WIDTH,
//         "Methodologies used to calculate or measure emissions other than those provided by the GHG Protocol. (Provide a reference or link to any non-GHG Protocol calculation tools used)",
//         data.methodologies, font, fontBold, lightBlue);
//       moveDown(80);
//     }
...
//       drawOrganizationalTable(currentPage, MARGIN, y, CONTENT_WIDTH, data.organizationalBoundaries, font, fontBold, lightBlue);
//       moveDown(150);
//     }
...
//       drawSingleRowTable(currentPage, MARGIN, y, CONTENT_WIDTH,
//         "If the reporting company's parent company does not report emissions, include an organizational diagram that clearly defines relationship of the reporting subsidiary as well as other subsidiaries",
//         data.organizationalDiagram, font, fontBold, lightBlue);
//       moveDown(120);
//     }

//     // ============================================
//     // STEP 3: DETAILED EMISSIONS AND FINAL SECTIONS
//     // ============================================

//     // DETAILED EMISSIONS BY SOURCE TYPE
//     const hasDetailedEmissions = data.stationaryCombustion || data.mobileCombustion || 
//       data.processSources || data.fugitiveSources || data.agriculturalSources ||
//       data.electricity || data.steam || data.heating || data.cooling ||
//       data.purchased_goods_services || data.capital_goods || data.fuel_energy_related ||
...
//       currentPage.drawText("DETAILED EMISSIONS BY SOURCE", {
//         x: MARGIN,
//         y,
//         size: 14,
//         font: fontBold
...
//         detailedEmissions.push({
//           label: "Scope 1: Direct Emissions from Owned/Controlled Operations",
//           value: "", subHeader: true
//         });
//         if (data.stationaryCombustion) detailedEmissions.push({
//           label: "a. Direct Emissions from Stationary Combustion", value: data.stationaryCombustion
//         });
//         if (data.mobileCombustion) detailedEmissions.push({
//           label: "b. Direct Emissions from Mobile Combustion", value: data.mobileCombustion
//         });
//         if (data.processSources) detailedEmissions.push({
//           label: "c. Direct Emissions from Process Sources", value: data.processSources
//         });
//         if (data.fugitiveSources) detailedEmissions.push({
//           label: "d. Direct Emissions from Fugitive Sources", value: data.fugitiveSources
//         });
//         if (data.agriculturalSources) detailedEmissions.push({
//           label: "e. Direct Emissions from Agricultural Sources", value: data.agriculturalSources
//         });
//       }
...
//         });
//         if (data.electricity) detailedEmissions.push({
//           label: "a. Indirect Emissions from Purchased/Acquired Electricity", value: data.electricity
//         });
//         if (data.steam) detailedEmissions.push({
//           label: "b. Indirect Emissions from Purchased/Acquired Steam", value: data.steam
//         });
//         if (data.heating) detailedEmissions.push({
//           label: "c. Indirect Emissions from Purchased/Acquired Heating", value: data.heating
//         });
//         if (data.cooling) detailedEmissions.push({
//           label: "d. Indirect Emissions from Purchased/Acquired Cooling", value: data.cooling
//         });
//       }
...
//         if (data.purchased_goods_services) detailedEmissions.push({
//           label: "1. Purchased Goods and Services", value: data.purchased_goods_services
//         });
//         if (data.capital_goods) detailedEmissions.push({
//           label: "2. Capital Goods", value: data.capital_goods
//         });
//         if (data.fuel_energy_related) detailedEmissions.push({
//           label: "3. Fuel- and Energy-Related Activities (not included in Scope 1 or 2)",
//           value: data.fuel_energy_related
//         });
//         if (data.upstream_transport_distribution) detailedEmissions.push({
//           label: "4. Upstream Transportation and Distribution", value: data.upstream_transport_distribution
//         });
//         if (data.waste_generated) detailedEmissions.push({
//           label: "5. Waste Generated in Operations", value: data.waste_generated
//         });
//         if (data.business_travel) detailedEmissions.push({
//           label: "6. Business Travel", value: data.business_travel
//         });
//         if (data.employee_commuting) detailedEmissions.push({
//           label: "7. Employee Commuting", value: data.employee_commuting
//         });
//         if (data.upstream_leased_assets) detailedEmissions.push({
//           label: "8. Upstream Leased Assets", value: data.upstream_leased_assets
//         });
//         if (data.downstream_transport_distribution) detailedEmissions.push({
//           label: "9. Downstream Transportation and Distribution", value: data.downstream_transport_distribution
//         });
//         if (data.processing_sold_products) detailedEmissions.push({
//           label: "10. Processing of Sold Products", value: data.processing_sold_products
//         });
//         if (data.use_sold_products) detailedEmissions.push({
//           label: "11. Use of Sold Products", value: data.use_sold_products
//         });
//         if (data.end_of_life_sold_products) detailedEmissions.push({
//           label: "12. End-of-Life Treatment of Sold Products", value: data.end_of_life_sold_products
//         });
//         if (data.downstream_leased_assets) detailedEmissions.push({
//           label: "13. Downstream Leased Assets", value: data.downstream_leased_assets
//         });
//         if (data.franchises) detailedEmissions.push({
//           label: "14. Franchises", value: data.franchises
//         });
//         if (data.investments) detailedEmissions.push({
//           label: "15. Investments", value: data.investments
//         });
//       }

//       const rowHeight = 20;
//       const totalHeight = detailedEmissions.length * rowHeight;
      
//       checkSpace(totalHeight + 50);
//       drawDetailedEmissionsTable(currentPage, MARGIN, y, CONTENT_WIDTH, detailedEmissions, font, fontBold, lightBlue);
//       moveDown(totalHeight + 20);
//     }
...
//       drawFacilityTableWithHeader(currentPage, MARGIN, y, CONTENT_WIDTH,
//         "Emissions disaggregated by facility (recommended for individual facilities with stationary combustion emissions over 10,000 mtCO2e)",
//         data.facilityEmissions, font, fontBold, lightBlue);
//       moveDown(120);
//     }
...
//     if (data.countryEmissions && data.countryEmissions.length > 0) {
//       checkSpace(150);
//       drawCountryTableWithHeader(currentPage, MARGIN, y, CONTENT_WIDTH,
//         "Emissions disaggregated by country",
//         data.countryEmissions, font, fontBold, lightBlue);
//       moveDown(120);
//     }

//     // OWN GENERATION EMISSIONS
//     if (data.ownGenerationEmissions) {
//       checkSpace(80);
//       drawSingleRowTable(currentPage, MARGIN, y, CONTENT_WIDTH,
//         "Emissions attributable to own generation of electricity, heat, or steam that is sold or transferred to another organization",
//         data.ownGenerationEmissions, font, fontBold, lightBlue);
//       moveDown(80);
//     }

//     // PURCHASED FOR RESALE EMISSIONS
//     if (data.purchasedForResaleEmissions) {
//       checkSpace(80);
//       drawSingleRowTable(currentPage, MARGIN, y, CONTENT_WIDTH,
//         "Emissions attributable to the generation of electricity, heat or steam that is purchased for re-sale to non-end users",
//         data.purchasedForResaleEmissions, font, fontBold, lightBlue);
//       moveDown(80);
//     }
...
//       drawSingleRowTable(currentPage, MARGIN, y, CONTENT_WIDTH,
//         "Emissions from GHGs not covered by the Kyoto Protocol (e.g., CFCs, NOx)",
//         data.nonKyotoEmissions, font, fontBold, lightBlue);
//       moveDown(80);
//     }

//     // EMISSION CHANGE CAUSES
//     if (data.emissionChangeCauses) {
//       checkSpace(100);
//       moveDown(20);
//       drawSingleRowTable(currentPage, MARGIN, y, CONTENT_WIDTH,
//         "Information on the causes of emissions changes that did not trigger a base year emissions recalculation (e.g., process changes, efficiency improvements, plant closures)",
//         data.emissionChangeCauses, font, fontBold, lightBlue);
//       moveDown(80);
//     }
...
//       drawSingleRowTable(currentPage, MARGIN, y, CONTENT_WIDTH,
//         "GHG emissions data for all years between the base year and the reporting year (including details of and reasons for recalculations, if appropriate)",
//         data.historicalEmissions, font, fontBold, lightBlue);
//       moveDown(80);
//     }
...
//       drawSingleRowTable(currentPage, MARGIN, y, CONTENT_WIDTH,
//         "Relevant ratio performance indicators (e.g. emissions per kilowatt-hour generated, sales, etc.)",
//         data.performanceIndicators, font, fontBold, lightBlue);
//       moveDown(80);
//     }
...
//       drawSingleRowTable(currentPage, MARGIN, y, CONTENT_WIDTH,
//         "An outline of any GHG management/reduction programs or strategies",
//         data.managementPrograms, font, fontBold, lightBlue);
//       moveDown(80);
//     }

//     // ADDITIONAL INFORMATION SECTION
//     const hasAdditionalInfo = data.contractualProvisions || data.externalAssurance ||
//                               data.inventoryQuality || data.sequestrationInfo;
...
//         drawSingleRowTable(currentPage, MARGIN, y, CONTENT_WIDTH,
//           "Information on any contractual provisions addressing GHG-related risks and obligations",
//           data.contractualProvisions, font, fontBold, lightBlue);
//         moveDown(80);
//       }
...
//         drawSingleRowTable(currentPage, MARGIN, y, CONTENT_WIDTH,
//           "An outline of any external assurance provided and a copy of any verification statement, if applicable, of the reported emissions data",
//           data.externalAssurance, font, fontBold, lightBlue);
//         moveDown(80);
//       }
...
//         drawSingleRowTable(currentPage, MARGIN, y, CONTENT_WIDTH,
//           "Information on the quality of the inventory (e.g., information on the causes and magnitude of uncertainties in emission estimates) and an outline of policies in place to improve inventory quality",
//           data.inventoryQuality, font, fontBold, lightBlue);
//         moveDown(80);
//       }
...
//         drawSingleRowTable(currentPage, MARGIN, y, CONTENT_WIDTH,
//           "Information on any GHG sequestration",
//           data.sequestrationInfo, font, fontBold, lightBlue);
//         moveDown(80);
//       }
//     }

//     // OFFSETS SECTION
//     const hasOffsetsData = (data.offsetsOutsideBoundary && data.offsetsOutsideBoundary.length > 0) ||
//                           (data.offsetsInsideBoundary && data.offsetsInsideBoundary.length > 0);
...
//       currentPage.drawText("INFORMATION ON OFFSETS", {
//         x: MARGIN,
//         y,
//         size: 14,
//         font: fontBold
...
//       if (data.offsetsOutsideBoundary && data.offsetsOutsideBoundary.length > 0) {
//         checkSpace(150);
//         drawOffsetsTableWithHeader(currentPage, MARGIN, y, CONTENT_WIDTH,
//           "Information on offsets that have been purchased or developed outside the inventory boundary",
//           data.offsetsOutsideBoundary, font, fontBold, lightBlue);
//         moveDown(120);
//       }
...
//         drawOffsetsTableWithHeader(currentPage, MARGIN, y, CONTENT_WIDTH,
//           "Information on reductions inside the inventory boundary that have been sold/transferred as offsets to a third party",
//           data.offsetsInsideBoundary, font, fontBold, lightBlue);
//         moveDown(120);
//       }
//     }
...
//   } catch (err) {
//     console.log(err);
//     return this.utility.response.init(res, false, "Internal server error while creating PDF", {
//       error: "INTERNAL_SERVER_ERROR",
//       details: err.message
//     }, 500);
//   }
...
generate_pdf = async (req, res) => {
  ...
  "verified": true,
  "verificationDate": "01/15/2024",
  "verifier": "Green Audit Corp",
  "verifierEmail": "contact@greenaudit.com",
  "verifierPhone": "+1-555-0123",
  "verifierAddress": "123 Audit Street, Green City, CA 90210",
  "exclusions": "Small office facilities under 1000 sq ft have been excluded from this inventory",
  ...
  "scope3Included": true,
  "scope3Activities": "Business travel, employee commuting, waste disposal, and purchased goods",
  "emissions": {
    "scope1Total": "1250.5",
    "scope1CO2": "1100.2",
    "scope1CH4": "50.3",
  ...
  "biogenicEmissions": "25.5",
  "baseYear": "2020",
  "baseYearPolicy": "Base year emissions are recalculated when structural changes exceed 5% of total emissions",
  "baseYearContext": "Acquisition of new manufacturing facility in Q2 2023 resulted in base year recalculation",
  "baseYearEmissions": {
    "scope1Total": "1000.0",
    "scope1CO2": "900.0",
    "scope1CH4": "50.0",
  ...
  "methodologies": "EPA emission factors for stationary combustion, IPCC guidelines for GWP values, IEA country-specific grid factors",
  "organizationalBoundaries": [
    {
      "entity": "Main Manufacturing Plant - Texas",
      "equityShare": "100%",
      "financialControl": "yes",
      "operationalControl": "yes"
    },
    {
      "entity": "Distribution Center A - California",
      "equityShare": "100%",
      "financialControl": "yes",
      "operationalControl": "yes"
    },
    {
      "entity": "Joint Venture Facility - Mexico",
      "equityShare": "50%",
      "financialControl": "no",
      "operationalControl": "no"
    }
  ],
  "organizationalDiagram": "Organizational diagram attached in Appendix A showing parent-subsidiary relationships",
  "stationaryCombustion": "800.0 mtCO2e ",
  "mobileCombustion": "250.5 mtCO2e ",
  "processSources": "150.0 mtCO2e ",
  "fugitiveSources": "50.0 mtCO2e ",
  "agriculturalSources": "0 mtCO2e ",
  "electricity": "750.3 mtCO2e ",
  "steam": "50.0 mtCO2e ",
  "heating": "30.0 mtCO2e ",
  "cooling": "20.0 mtCO2e ",
  "purchased_goods_services": "20.0 mtCO2e ",
  "capital_goods": "20.0 mtCO2e ",
  "fuel_energy_related": "20.0 mtCO2e ",
  "upstream_transport_distribution": "20.0 mtCO2e ",
  "waste_generated": "20.0 mtCO2e ",
  "business_travel": "20.0 mtCO2e ",
  "employee_commuting": "20.0 mtCO2e ",
  "upstream_leased_assets": "20.0 mtCO2e ",
  "downstream_transport_distribution": "20.0 mtCO2e ",
  "processing_sold_products": "20.0 mtCO2e ",
  "use_sold_products": "20.0 mtCO2e ",
  "end_of_life_sold_products": "20.0 mtCO2e ",
  "downstream_leased_assets": "20.0 mtCO2e ",
  "franchises": "20.0 mtCO2e ",
  "investments": "20.0 mtCO2e ",
  "facilityEmissions": [
    {
      "facility": "Main Manufacturing Plant - Texas",
      "scope1Emissions": "650.0 mtCO2e"
    },
    {
      "facility": "Distribution Center - California",
      "scope1Emissions": "200.5 mtCO2e"
    },
    {
      "facility": "Office Complex - New York",
      "scope1Emissions": "150.0 mtCO2e"
    },
    {
      "facility": "Warehouse - Illinois",
      "scope1Emissions": "250.0 mtCO2e"
    }
  ],
  "countryEmissions": [
    {
      "country": "United States",
      "emissions": "2000.0 mtCO2e (Scope 1, 2 & 3)"
    },
    {
      "country": "Mexico",
      "emissions": "100.0 mtCO2e (Scope 1 only)"
    },
    {
      "country": "Canada",
      "emissions": "50.0 mtCO2e (Scope 2 only)"
    }
  ],
  "ownGenerationEmissions": "50.0 mtCO2e from on-site solar panels with excess energy sold to grid",
  "purchasedForResaleEmissions": "Not applicable to our business operations",
  "nonKyotoEmissions": "5.0 mt NOx from combustion processes, 2.0 mt CFCs from legacy equipment",
  "emissionChangeCauses": "Installation of energy-efficient HVAC systems reduced emissions by 15%. LED lighting retrofit completed in Q3. Process optimization in manufacturing reduced fuel consumption by 8%.",
  "historicalEmissions": "2020: 1700 mtCO2e | 2021: 1850 mtCO2e | 2022: 1950 mtCO2e | 2023: 2100 mtCO2e | 2024: 2550 mtCO2e",
  "performanceIndicators": "0.45 mtCO2e per $1M revenue | 2.1 mtCO2e per employee | 3.5 mtCO2e per 1000 units produced",
  "managementPrograms": "Energy efficiency program targeting 20% reduction by 2030. Renewable energy procurement strategy to reach 50% renewable electricity by 2028. Employee engagement program for emission reduction.",
  "contractualProvisions": "All major supplier contracts include emissions reduction clauses and annual reporting requirements. Transportation contracts specify use of low-emission vehicles where feasible.",
  "externalAssurance": "Limited assurance provided by Green Audit Corp. Full verification statement attached as Appendix B. No material misstatements identified.",
  "inventoryQuality": "Uncertainty estimated at ±5% for Scope 1, ±8% for Scope 2, ±15% for Scope 3. Annual third-party verification in place. Data quality management system follows ISO 14064-1 standards.",
  "sequestrationInfo": "Company forest offset program in partnership with local conservation group sequesters approximately 100 mtCO2e annually. 500 acres of reforested land managed.",
  "offsetsOutsideBoundary": [
    {
      "quantity": "150.0",
      "projectType": "Reforestation project in Amazon Basin",
      "verified": "Yes - Verified Carbon Standard (VCS)"
    },
    {
      "quantity": "75.0",
      "projectType": "Wind farm development in Midwest US",
      "verified": "Yes - Gold Standard certified"
    }
  ],
  "offsetsInsideBoundary": [
    {
      "quantity": "50.0",
      "projectType": "On-site solar installation renewable energy credits",
      "verified": "Yes - Regional Greenhouse Gas Initiative (RGGI)"
    },
    {
      "quantity": "25.0",
      "projectType": "Energy efficiency improvements sold as credits",
      "verified": "Yes - California Air Resources Board"
    }
  ]
};
    const pdfDoc = await PDFDocument.create();
